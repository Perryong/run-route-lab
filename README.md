...
Use **Health overview** in the header to see daily activity, full timestamped heart-rate and stress readings, Body Battery samples, sleep score and need, exact sleep window and stages, overnight heart rate/HRV/stress, respiration, SpO₂, skin temperature, and the measurements supported by your watch. Select a day using the date picker or any seven-day chart bar. Use **Expand** on the heart-rate or stress chart for a zoomable, horizontally scrollable timeline. Running filters do not change daily health metrics. All-day distance is separate from running distance. Today may be incomplete until the next watch sync.
