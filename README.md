...
Use **Health overview** in the header to see daily steps, Garmin daily total distance, resting heart rate, mean of available heart-rate samples, daily maximum heart rate, and seven days of step history. Select a day using the date picker or a chart bar. Running filters do not change daily health metrics. All-day distance is separate from running distance. Today may be incomplete until the next watch sync.

Automatic syncing now retrieves the last seven calendar days on each run, using Singapore time for the date window. Public output is `site/data/health.json`. Set repository variable `PUBLISH_HEALTH=false` to stop daily-health fetching and publish an empty daily dataset; previously downloaded copies cannot be recalled. Run average/max heart rate stays with activity statistics. Recorded samples are averaged without imputing missing readings; this is not necessarily Garmin Connect's own daily average. No clinical status or diagnosis is inferred.
